...
function rivalsSaidNoEnvit(intents: BotIntents, player: PlayerId): boolean {
  const playerTeam = teamOf(player);
  return Object.entries(intents.chatSignals ?? {}).some(([p, phrases]) => {
    if (teamOf(Number(p) as PlayerId) === playerTeam) return false;
    return phrases.includes("no");
  });
}
...
async function insertBotChat(
  roomId: string,
  seat: PlayerId,
  phraseId: ChatPhraseId,
  state?: MatchState | null,
) {
  // Compute phrase variables server-side from the authoritative state so the
  // client doesn't need access to opponents' real hands. Currently only
  // "si-tinc-n" carries a variable (the speaker's true envit value).
  let vars: Record<string, unknown> | null = null;
  if (phraseId === "si-tinc-n" && state) {
    vars = { n: playerTotalEnvit(state.round, seat) };
  }
  await admin.from("room_chat").insert({ room_id: roomId, seat, phrase_id: phraseId, vars });
}
...
function botReplyToHumanQuestion(
  ...
  const r = state.round;
  // Si l'envit ja s'ha cantat (envitState.kind !== "none") o s'ha resolt
  // en aquesta ronda, el bot NO pot tornar a dir "Envida!": com a molt
  // diu "Sí" (afirmant que té envit). Així evitem un segon "Envida!" del
  // mateix equip.
  const envitAlreadyCalled = r.envitState.kind !== "none" || r.envitResolved;
  const noEnvida = (phrase: ChatPhraseId): ChatPhraseId =>
    envitAlreadyCalled && phrase === "envida" ? "si" : phrase;
  if (question === "tens-envit") {
    const envit = playerTotalEnvit(r, botSeat);
    if (envit >= 31) return noEnvida(Math.random() < 0.5 ? "envida" : "si");
  ...
  if (question === "vols-tornar-envidar") {
    ...
    if (r.envitState.kind === "pending") {
      ...
      for (let i = 0; i < 4; i++) {
        if (p === botSeat) { manoPriorityOverCaller = true; break; }
        if (p === caller) { manoPriorityOverCaller = false; break; }
        p = ((p + 1) % 4) as PlayerId;
      }
    }
    if (envit >= 33 || (envit === 32 && manoPriorityOverCaller)) return "si";
    return "no";
  }
...
async function decideOnlineBotAction(
  ...
  if (intents.botFlow && intents.botFlow.actor === actor && intents.botFlow.id === flowId) {
    ...
    switch (intents.botFlow.kind) {
      case "peu-info": {
        ...
        await insertBotChat(roomId, speaker, phrase, state);
      ...
      case "consult-question": {
        ...
        await insertBotChat(roomId, actor, question, state);
      ...
      case "consult-answer": {
        ...
        await insertBotChat(roomId, partner, answer, state);
        ...
        scheduleBotFlow(intents, {
          ...
          dueAt: dueIso(decideDelayMs),
            payload: {
              ...
              reenvidarAnswer: question === "vols-tornar-envidar" ? answer : undefined,
            },
        });
        return null;
      }
      case "consult-decide": {
        ...
        if (payload.question === "vols-tornar-envidar") {
          const myEnvitNow = playerTotalEnvit(state.round, actor);
          const acts = legalActions(state, actor);
          const renvit = acts.find(
            (a) => a.type === "shout" && (a.what === "renvit" || a.what === "falta-envit"),
          );
          const vull = acts.find(
            (a) => a.type === "shout" && a.what === "vull",
          );
          let manoPriorityOverCaller = false;
          if (state.round.envitState.kind === "pending") {
            const caller = state.round.envitState.calledBy;
            let p: PlayerId = state.round.mano;
            for (let i = 0; i < 4; i++) {
              if (p === actor) { manoPriorityOverCaller = true; break; }
              if (p === caller) { manoPriorityOverCaller = false; break; }
              p = ((p + 1) % 4) as PlayerId;
            }
          }
          ...
          const reenvidarAnswer = payload.reenvidarAnswer as ChatPhraseId | undefined;
          if ((reenvidarAnswer === "si" || reenvidarAnswer === "si-tinc-n" || reenvidarAnswer === "envida") && canRaiseNow && renvit) {
            return renvit;
          }
          if (myEnvitNow >= 31 && vull) {
            return vull;
          }
        }
      ...
      case "second-wait": {
        ...
        const partner = Number(payload.partner) as PlayerId;
        if (stage === "ask") {
          await insertBotChat(roomId, actor, "vols-envide", state);
          recordBotChat(intents, actor, "vols-envide");
          if (seatKinds[partner] === "bot") {
            const answer = partnerAnswerFor(state, partner, "vols-envide", bluffRate, { rivalsSaidNoEnvit: rivalsSaidNoEnvit(intents, partner) });
            scheduleBotFlow(intents, {
              id: flowId,
              actor,
              kind: "second-wait",
        ...
        if (stage === "partner-instruction") {
          ...
          await insertBotChat(roomId, partner, instruction, state);
  ...
  // Davant d'una falta-envit pendent, o si l'envit ja s'ha pujat (renvit
  // o superior), els bots no han de preguntar res del joc de cartes
  // (puc-anar/que-tens) ni "Vols tornar a envidar?": ja s'ha reenvidat.
  // Pregunten al company "Quant envit tens?" i el company respon amb el
  // seu valor real ("Tinc {n}", calculat al client). Després el bot
  // decideix amb la regla corresponent de `decideEnvitResponse`.
  ...
  // Davant d'un envit del rival (no falta), si el bot té 31/32/33 i és
  // qui ha de respondre, HA de preguntar primer al company "Vols tornar a
  // envidar?". Després: si el company diu "no", farà "vull"; si diu "sí",
  // renvidarà / tirarà falta si és legal. Per això no fem cap renvit directe.
  // En "match point" de cama l'envit només pot valdre 1 i no es pot
  // pujar (renvit / falta-envit), així que el bot no consulta el company:
  // decideix sense preguntar.
  if (
    firstPendingResponseActor(state, actor) &&
    r.envitState.kind === "pending" &&
    !envitAlreadyRaised &&
    !isCamaMatchPoint(state)
  ) {
    ...
    if (myEnvitNow >= 31 && canRaiseOrAccept && !partnerRejected) {
      return scheduleBotFlow(intents, {
        id: flowId,
        actor,
        kind: "consult-question",
        dueAt: dueIso(questionDelayMs),
        payload: { question: "vols-tornar-envidar", partner: partnerSeat },
      });
    }
  }
  ...
  if (
    ...
    if (!asEspasesPlayedFirstTrick(state.round)) {
      await insertBotChat(roomId, actor, "a-tu", state);
    ...
    if (playActs.length > 0 && hand.length > 0) {
      ...
      const matchAct = playActs.find((a) => a.cardId === lowest.id) ?? playActs[0]!;
      return scheduleBotFlow(intents, {
        id: flowId,
        actor,
        kind: "action",
        dueAt: dueIso(decideDelayMs),
        payload: { action: matchAct },
      });
    }
  }
...
/**
 * Creates a fresh MatchState for the room and decides whether to start it
 * immediately ("playing") or keep the room open ("lobby") waiting for more
 * humans to seat. Returns the chosen status.
 *
 * Used by both rematchStay (after a player presses "Nova partida" at the
 * end of a match) and leaveRoom (after a player abandons at the end of a
 * match) to either auto-start a new match if the table is full of humans,
 * or reopen the table so other players can fill empty seats.
 */
async function resetMatchAndMaybeStart(room: any): Promise<"playing" | "lobby"> {
  const nowIso = new Date().toISOString();
  const initialMano = room.initial_mano as PlayerId;
  const firstDealer = (((initialMano + 3) % 4) as PlayerId);
  const matchState = createMatch({
  ...
  const { data: occupants } = await admin
    ...
    .select("seat")
    .eq("room_id", room.id);
  const occupiedSeats = new Set((occupants ?? []).map((p: any) => p.seat));
  const allHumansFilled = humanSeats.every((s) => occupiedSeats.has(s));
  const status: "playing" | "lobby" = allHumansFilled ? "playing" : "lobby";
  const turnStartedAt = status === "playing"
    ? computeTurnStartedAt(null, matchState, null)
    : null;
  await admin.from("rooms").update({
    status,
    match_state: matchState,
...
const handlers: Record<string, (data: any) => Promise<unknown>> = {
  ...
  async joinRoom(d) {
    ...
    // Auto-start: if all human seats are now filled, kick off a fresh match
    // immediately so players don't have to wait for the host to press start.
    const humanSeatIdx = seatKinds
      .map((k, i) => (k === "human" ? i : -1))
      .filter((i) => i >= 0);
    const newOccupied = new Set([...usedSeats, chosenSeat]);
    if (humanSeatIdx.every((s) => newOccupied.has(s))) {
      await resetMatchAndMaybeStart(room);
    }
  ...
  async leaveRoom(d) {
    ...
    // En lobby o partida acabada, traiem físicament el jugador del seient
    // per a que es puga reocupar. En partida en curs, només marquem offline.
    const seatFreeing = room.status === "lobby" || room.status === "finished";
    if (seatFreeing) {
    ...
    // Si en lobby/finished ja no queda cap humà a la sala, l'abandonem perquè
    // quede totalment lliure.
    if (seatFreeing) {
      ...
      // Si encara queden humans, si l'host se'n va el reassignem al primer
      // restant; i si veníem de "finished", reiniciem el match i tornem la
      // taula a "lobby" perquè altres jugadors puguen omplir el seient.
      if (isHost) {
    ...
    if (isHost) {
      // Partida en curs i l'host se'n va: marquem la taula com abandonada.
  ...
  /**
   * Quan un jugador prem "Nova partida" al final d'una partida, indica
   * que vol quedar-se a la taula. Si la taula està plena d'humans,
   * comencem la nova partida immediatament. Si algú ha abandonat,
   * retornem la taula a "lobby" perquè altres puguen ocupar el seient
   * lliure; quan s'ompla, l'auto-start de joinRoom encetarà la partida.
   */
  async rematchStay(d) {
    const { data: room } = await admin.from("rooms").select("*").eq("id", d.roomId).maybeSingle();
    if (!room) throw new Error("Sala no trobada");
    if (room.status !== "finished" && room.status !== "lobby") {
      throw new Error("La partida encara no ha acabat");
    }
    // Ens assegurem que el jugador continua marcat com a online.
    await admin.from("room_players")
      .update({ is_online: true, last_seen: new Date().toISOString() })
      .eq("room_id", room.id)
      .eq("device_id", d.deviceId);
    // Si l'amfitrió havia eixit (host hostless), aquest jugador en pren el relleu.
    const HOSTLESS = "__lobby__";
    if (room.host_device === HOSTLESS) {
      await admin.from("rooms").update({ host_device: d.deviceId, updated_at: new Date().toISOString() }).eq("id", room.id);
      room.host_device = d.deviceId;
    }
    if (room.status === "finished") {
      const status = await resetMatchAndMaybeStart(room);
      return { ok: true, status };
    }
    // Ja estem en lobby (algú ha abandonat abans). Comprovem si ara està plena.
    const status = await resetMatchAndMaybeStart(room);
    return { ok: true, status };
  },
  ...
  async sendChatPhrase(d) {
      ...
      if (botFlow.kind === "consult-decide" && botInfoPhrases().includes(d.phraseId as ChatPhraseId)) {
        intents.botFlow = {
          ...
          payload: {
            ...
            reenvidarAnswer:
              botFlow.payload.question === "vols-tornar-envidar"
                ? (d.phraseId as ChatPhraseId)
                : botFlow.payload.reenvidarAnswer,
          },
        };
    ...
    if (botAnswer) {
      ...
      await insertBotChat(room.id, partner, botAnswer, state);
      recordBotChat(intents, partner, botAnswer);
      if (d.phraseId === "vols-tornar-envidar" && botAnswer === "si") {
        stateChanged = (await tryDispatch("falta-envit")) || stateChanged;
        stateChanged = (await tryDispatch("renvit")) || stateChanged;
      }
    }
