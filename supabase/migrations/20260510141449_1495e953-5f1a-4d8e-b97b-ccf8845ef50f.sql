...
CREATE POLICY "user_stats_no_client_write" ON public.user_stats
  FOR ALL TO authenticated, anon USING (false) WITH CHECK (false);

-- =====================================================
-- FRIENDSHIPS
-- status: 'pending' | 'accepted'
-- Stored in canonical form: row exists once per pair, with requested_by
-- distinguishing direction. To avoid duplicates we order ids.
-- =====================================================
...
CREATE POLICY "friendships_no_client_write" ON public.friendships
  FOR ALL TO authenticated, anon USING (false) WITH CHECK (false);
...
INSERT INTO public.profiles (user_id, display_name, friend_code, email)
SELECT u.id,
       COALESCE(NULLIF(u.raw_user_meta_data->>'display_name',''), split_part(u.email,'@',1), 'Jugador'),
...
-- =====================================================
-- LEVEL HELPER
-- Level N requires N*100 XP increment. Cumulative threshold
-- to reach level N = 100 * N*(N-1)/2.
-- =====================================================
...
CREATE OR REPLACE FUNCTION public.record_match_result(
  p_won boolean,
  p_human_opponents integer,
  p_bot_opponents integer
)
...
BEGIN
  ...
  -- cap: max 3 opponents in a 4-player match
  IF p_human_opponents + p_bot_opponents > 3 THEN
    p_bot_opponents := GREATEST(0, 3 - p_human_opponents);
  END IF;

  IF p_won THEN base_xp := 50; ELSE base_xp := 10; END IF;
  -- humans give double XP vs bots
  IF p_won THEN
    opp_xp := p_human_opponents * 20 + p_bot_opponents * 10;
  ELSE
    opp_xp := p_human_opponents * 5 + p_bot_opponents * 2;
  END IF;
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    max_streak = GREATEST(
      public.user_stats.max_streak,
      CASE WHEN p_won THEN public.user_stats.current_streak + 1 ELSE public.user_stats.max_streak END
    ),
  ...
  ON CONFLICT (user_a, user_b) DO UPDATE
    SET status = CASE
      WHEN public.friendships.status = 'pending'
       AND public.friendships.requested_by <> EXCLUDED.requested_by
      THEN 'accepted'
