...
CREATE OR REPLACE FUNCTION public.record_match_result(
  p_won boolean, p_human_opponents integer, p_bot_opponents integer
) RETURNS public.user_stats
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  uid uuid := auth.uid();
  base_xp integer; opp_xp integer; total_xp integer;
...
BEGIN
  ...
  ON CONFLICT (user_a, user_b) DO UPDATE
    SET status = CASE WHEN public.friendships.status = 'pending' AND public.friendships.requested_by <> EXCLUDED.requested_by THEN 'accepted' ELSE public.friendships.status END,
        updated_at = now()
  RETURNING * INTO result;
  RETURN result;
END;
