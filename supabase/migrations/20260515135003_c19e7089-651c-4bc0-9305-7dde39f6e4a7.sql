...
CREATE TABLE IF NOT EXISTS public.rooms (
  ...
  target_cama integer NOT NULL DEFAULT 12,
  turn_timeout_sec integer NOT NULL DEFAULT 30,
  pending_proposal jsonb,
  CONSTRAINT rooms_target_cama_chk CHECK (target_cama IN (9, 12)),
  CONSTRAINT rooms_turn_timeout_sec_chk CHECK (turn_timeout_sec IN (15, 30, 45, 60))
);
...
CREATE TABLE IF NOT EXISTS public.player_profiles (
  ...
  bot_honesty text NOT NULL DEFAULT 'sincero' CHECK (bot_honesty IN ('sincero', 'pillo', 'mentider')),
  updated_at timestamptz NOT NULL DEFAULT now(),
  created_at timestamptz NOT NULL DEFAULT now()
);
...
DO $$ BEGIN ALTER PUBLICATION supabase_realtime ADD TABLE public.player_profiles; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
